...
app.post("/api/detail-pembelian", async (req, res) => {
  const {
    email,
    id,
    pembelian_id,
  ...
  // id = detail_pembelian_id dari lokal

  if (!email || !id || !pembelian_id || !produk_id) {
    return res.status(400).json({ message: "invalid payload" });
  }
  ...
  try {
    ...
    // 2️⃣ Cek apakah detail sudah ada
    const [existing] = await db.query(
      `
      SELECT id
      FROM detail_pembelian
      WHERE user_id = ? AND detail_pembelian_id = ?
      LIMIT 1
      `,
      [user_id, id]
    );
    ...
    if (existing.length > 0) {
      ...
      await db.query(
        ...
        WHERE user_id = ? AND detail_pembelian_id = ?
        `,
        [pembelian_id, produk_id, jumlah, user_id, id]
      );
    } else {
      // 3b️⃣ INSERT
      await db.query(
        ...
        INSERT INTO detail_pembelian
          (user_id, detail_pembelian_id, pembelian_id, produk_id, jumlah)
        VALUES
          (?, ?, ?, ?, ?)
        `,
        [user_id, id, pembelian_id, produk_id, jumlah]
      );
    }

    res.json({ status: "ok" });
  } catch (err) {
    console.error("ERROR /api/detail-pembelian:", err);
    res.status(500).json({ message: "server error" });
  }
});

app.post("/api/detail-pesanan-online", async (req, res) => {
  const {
    email,
    id,
    pesanan_online_id,
  ...
  // id = detail_pesanan_online_id dari lokal / bot

  if (!email || !id || !pesanan_online_id || !produk_id) {
    return res.status(400).json({ message: "invalid payload" });
  }
  ...
  try {
    ...
    // 2️⃣ Cek apakah detail pesanan sudah ada
    const [existing] = await db.query(
      `
      SELECT id
      FROM detail_pesanan_online
      WHERE user_id = ? AND detail_pesanan_online_id = ?
      LIMIT 1
      `,
      [user_id, id]
    );
    ...
    if (existing.length > 0) {
      ...
      await db.query(
        ...
        WHERE user_id = ? AND detail_pesanan_online_id = ?
        `,
        [pesanan_online_id, produk_id, jumlah, subtotal, user_id, id]
      );
    } else {
      // 3b️⃣ INSERT
      await db.query(
        ...
        INSERT INTO detail_pesanan_online
          (user_id, detail_pesanan_online_id, pesanan_online_id, produk_id, jumlah, subtotal)
        VALUES
          (?, ?, ?, ?, ?, ?)
        `,
        [user_id, id, pesanan_online_id, produk_id, jumlah, subtotal]
      );
    }

    res.json({ status: "ok" });
  } catch (err) {
    console.error("ERROR /api/detail-pesanan-online:", err);
    res.status(500).json({ message: "server error" });
  }
});
